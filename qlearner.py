...
class QLearner(object):
    def __init__(self, num_states=100, num_actions=3, alpha=0.2, gamma=0.9, rar=0.5, radr=0.99, dyna=0, verbose=False):
        ...
        # Initialize Q table
        self.q = uniform(-1, 1, size=(num_states, num_actions))

        # Initialize T and R tables for dyna
        if self.dyna != 0:
            self.Tc = ndarray(shape=(num_states, num_actions, num_states))
            self.Tc.fill(0.00001)
            self.T = self.Tc / self.Tc.sum(axis=2, keepdims=True)
            self.R = ndarray(shape=(num_states, num_actions))
            self.R.fill(-1.0)
    ...
    def query(self, s_prime, r):
        ...
        self.q[self.s, self.a] = (1 - self.alpha) * self.q[self.s, self.a] + self.alpha * (
            r + self.gamma * max(self.q[s_prime,]))

        if rand.random() <= self.rar:
            action = rand.randint(0, self.num_actions - 1)
        else:
            action = argmax(self.q[s_prime,])
        ...
        if self.dyna != 0:
            ...
            for i in range(0, self.dyna):
                ...
                a_dyna = randint(low=0, high=self.num_actions)
                s_dyna = randint(low=0, high=self.num_states)
                # infer s' from T
                s_prime_dyna = multinomial(1, self.T[s_dyna, a_dyna,]).argmax()
                # compute R from s and a
                r = self.R[s_dyna, a_dyna]
                # update Q
                self.q[s_dyna, a_dyna] = (1 - self.alpha) * self.q[s_dyna, a_dyna] + self.alpha * (r + self.gamma * max(self.q[s_prime_dyna,]))
